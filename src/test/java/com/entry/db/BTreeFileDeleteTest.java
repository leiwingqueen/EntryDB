...
public class BTreeFileDeleteTest extends SimpleDbTestBase {
    ...
    public void testStealFromLeftLeafPage() throws Exception {
        ...
        assertTrue(sibling.reverseIterator().next().getField(keyField).compare(Predicate.Op.LESS_THAN_OR_EQ,
    ...
    public void testStealFromRightLeafPage() throws Exception {
        ...
        assertTrue(page.getNumTuples() == totalTuples / 2 || page.getNumTuples() == totalTuples / 2 + 1);
        assertTrue(sibling.getNumTuples() == totalTuples / 2 || sibling.getNumTuples() == totalTuples / 2 + 1);
        assertTrue(page.reverseIterator().next().getField(keyField).compare(Predicate.Op.LESS_THAN_OR_EQ,
    ...
    public void testMergeLeafPages() throws Exception {
        ...
        Field minKey = (siblingKey.compare(Predicate.Op.LESS_THAN, parentKey) ? siblingKey : parentKey);
        entry.setKey(minKey);
        ...
        empty.mergeLeafPages(tid, leftPage, rightPage, parent, entry);
        ...
        assertEquals(rightPageId.getPageNumber(), empty.getEmptyPageNo(tid));
    }
    ...
    public void testStealFromLeftInternalPage() throws Exception {
        ...
        dirtypages.put(parentId, parent);
        empty.stealFromLeftInternalPage(tid, page, sibling, parent, entry);
        ...
        assertTrue(sibling.reverseIterator().next().getKey().compare(Predicate.Op.LESS_THAN_OR_EQ,
                page.iterator().next().getKey()));

        // is the parent key reasonable?
        assertTrue(parent.iterator().next().getKey().compare(Predicate.Op.LESS_THAN_OR_EQ, page.iterator().next().getKey()));
        assertTrue(parent.iterator().next().getKey().compare(Predicate.Op.GREATER_THAN_OR_EQ, sibling.reverseIterator().next().getKey()));
    ...
    public void testStealFromRightInternalPage() throws Exception {
        ...
        dirtypages.put(siblingId, sibling);
        dirtypages.put(parentId, parent);
        empty.stealFromRightInternalPage(tid, page, sibling, parent, entry);
        ...
        // are the keys in the left page less than the keys in the right page?
        assertTrue(page.reverseIterator().next().getKey().compare(Predicate.Op.LESS_THAN_OR_EQ,
                sibling.iterator().next().getKey()));

        // is the parent key reasonable?
        assertTrue(parent.iterator().next().getKey().compare(Predicate.Op.LESS_THAN_OR_EQ, sibling.iterator().next().getKey()));
        assertTrue(parent.iterator().next().getKey().compare(Predicate.Op.GREATER_THAN_OR_EQ, page.reverseIterator().next().getKey()));
    ...
    public void testMergeInternalPages() throws Exception {
        ...
        // dirtypages.put(rightPageId, rightPage);
        // dirtypages.put(parentId, parent);
        empty.mergeInternalPages(tid, leftPage, rightPage, parent, entry);
