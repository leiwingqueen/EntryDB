...
public class BTreeInternalPageTest extends SimpleDbTestBase {
	...
	@Test public void testIterator() throws Exception {
		...
		entries.sort(new BTreeFileEncoder.EntryComparator());

		int row = 0;
		while (it.hasNext()) {
			BTreeEntry e = it.next();

			Assert.assertEquals(entries.get(row).getKey(), e.getKey());
			row++;
		}
	}
	...
	@Test public void testReverseIterator() throws Exception {
		...
		entries.sort(new BTreeFileEncoder.ReverseEntryComparator());
