...
import com.entry.db.index.BTreeRootPtrPage;
...
public class BTreeDeadlockTest extends SimpleDbTestBase {
	...
	@Before public void setUp() throws Exception {
		...
		DbFileIterator it = bf.indexIterator(tid, new IndexPredicate(Predicate.Op.EQUALS, new IntField(item1)));
		it.open();
		...
		it = bf.indexIterator(tid, new IndexPredicate(Predicate.Op.EQUALS, new IntField(item2)));
		it.open();
		tuples.clear();
		while(it.hasNext()) {
			tuples.add(it.next());
	...
	public BTreeUtility.BTreeWriter startWriter(TransactionId tid, 
		...
		BTreeUtility.BTreeWriter bw = new BTreeUtility.BTreeWriter(tid, bf, item, count);
		bw.start();
		return bw;
	}
	...
	@Test public void testReadWriteDeadlock() throws Exception {
		...
		BTreeUtility.BTreeWriter writer1 = startWriter(tid1, item1, count1);
		BTreeUtility.BTreeWriter writer2 = startWriter(tid2, item2, count2);
