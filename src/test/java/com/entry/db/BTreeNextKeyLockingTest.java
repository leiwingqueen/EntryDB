...
public class BTreeNextKeyLockingTest extends SimpleDbTestBase {
	...
	public void nextKeyLockingTestLessThan() throws Exception {
		...
		while(count == 0) {
			...
			ipred = new IndexPredicate(Predicate.Op.EQUALS, key);
		...
		ipred = new IndexPredicate(Predicate.Op.LESS_THAN_OR_EQ, key);
		fit = bigFile.indexIterator(tid, ipred);
		fit.open();
		...
		BTreeUtility.BTreeWriter bw1 = new BTreeUtility.BTreeWriter(tid1, bigFile, ((IntField) key).getValue(), 1);
		bw1.start();
	...
	public void nextKeyLockingTestGreaterThan() throws Exception {
		...
		ipred = new IndexPredicate(Predicate.Op.GREATER_THAN_OR_EQ, key);
		fit = bigFile.indexIterator(tid, ipred);
		fit.open();
		int keyCountBefore = 0;
		while(fit.hasNext()) {
