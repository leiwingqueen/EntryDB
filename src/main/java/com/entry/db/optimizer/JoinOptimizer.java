...
public class JoinOptimizer {
    ...
    public static final double ESTIMATE_FRACTION = 0.3D;
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp == Predicate.Op.EQUALS) {
            ...
            return (int) (card1 * card2 * ESTIMATE_FRACTION);
        }
    }
    ...
    // dfs. implement by leiwingqueen
    public static <T> Set<Set<T>> enumerateSubsets2(List<T> v, int size) {
        List<Integer> res = new ArrayList<>();
        dfs(0, 0, size, v.size(), res);
        Set<Set<T>> setRes = new HashSet<>();
        for (Integer num : res) {
    ...
    private static void dfs(int state, int idx, int size, int n, List<Integer> res) {
        if (size == 0) {
            res.add(state);
            return;
        }
        ...
        dfs(state, idx + 1, size, n, res);
        dfs(state | (1 << idx), idx + 1, size - 1, n, res);
    }
